...
class BiometricGUI:
    ...
    def setup_ui(self):
        # Título principal
        title_label = tk.Label(self.root, text="Sistema Biométrico", 
        ...
        add_user_btn = tk.Button(button_frame, text="1. Agregar Nuevo Usuario", 
                                font=("Arial", 16, "bold"), bg="#3498db", fg="white",
                                width=25, height=2, command=self.add_new_user)
        add_user_btn.pack(pady=20)
        
        # Botón para autenticación biométrica
        auth_btn = tk.Button(button_frame, text="2. Autenticación Biométrica", 
                            font=("Arial", 16, "bold"), bg="#e74c3c", fg="white",
                            width=25, height=2, command=self.biometric_auth)
        ...
        self.instruction_label = tk.Label(self.root, text="", font=("Arial", 14), 
                                         bg="#f0f0f0", fg="#2c3e50")
    ...
    def add_new_user(self):
        ...
        user_name = simpledialog.askstring("Nuevo Usuario", "Ingrese el nombre del usuario:")
        if not user_name:
            return
    ...
    def start_capture(self):
        ...
        cancel_btn = tk.Button(self.control_frame, text="Cancelar Captura", 
                              bg="#e74c3c", fg="white", command=self.stop_capture)
        cancel_btn.pack()
    ...
    def capture_faces(self):
        ...
        while self.is_capturing and self.cap.isOpened():
            ...
            if len(faces) > 0:
                ...
                # Instrucción actual
                instruction = f"Mire hacia: {self.poses[self.pose_index]} ({self.img_count % 5 + 1}/5)"
                cv2.putText(frame, instruction, (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 
                           0.8, (0, 0, 255), 2)
                
                # Actualizar instrucción en GUI
                self.root.after(0, lambda: self.instruction_label.config(text=instruction))
                ...
                if time.time() - last_capture > 2:
                    ...
                    # Cambiar pose después de 5 imágenes
    ...
    def generate_lbph_model(self):
        ...
        model = cv2.face.LBPHFaceRecognizer_create(radius=2, neighbors=16, grid_x=8, grid_y=8)
    ...
    def biometric_auth(self):
        ...
        if not self.check_models_exist():
            messagebox.showerror("Error", "No se encontraron usuarios registrados. Por favor agregue un usuario primero.")
            return
        ...
        # Crear botón de cancelar
        cancel_btn = tk.Button(self.control_frame, text="Cancelar Autenticación", 
                              bg="#e74c3c", fg="white", command=self.stop_auth)
        cancel_btn.pack()
        ...
        while self.is_authenticating and self.cap.isOpened():
            ...
            for face_data in faces:
                ...
                for model_data in self.auth_models:
                    ...
                    # Test FaceNet
                    similarities = [self.cosine_similarity(face_emb, emb) for emb in model_data['embeddings']]
                    facenet_score = max(similarities) * 100
                    
                    # Usar el mejor score
                    combined_score = max(lbph_score, facenet_score)
                    
                    if combined_score > best_score and combined_score > 60:  # Umbral mínimo
...
if __name__ == "__main__":
    ...
    root.protocol("WM_DELETE_WINDOW", lambda: (app.stop_capture() if app.is_capturing else None, 
